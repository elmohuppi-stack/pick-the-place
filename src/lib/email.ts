...
export async function sendProposalInvite(
  ...
  if (!resend) {
    console.log(
      `[EMAIL MOCK] Proposal invite to ${email}: ${APP_URL}/propose?token=${token}`,
    );
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        ...
        <p>Du bist eingeladen, einen Ort für unser nächstes Jahrestreffen vorzuschlagen.</p>
...
export async function sendVoteInvite(
  ...
  if (!resend) {
    console.log(
      `[EMAIL MOCK] Vote invite to ${email}: ${APP_URL}/vote?token=${token}`,
    );
    return { success: true, mocked: true };
  }
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
        <h1 style="color: #4f46e5;">Hallo ${name}!</h1>
        <p>Runde ${roundNumber} der Ortswahl ist gestartet!</p>
